...
DynamicAllocator::DynamicAllocator() noexcept {}
DynamicAllocator::DynamicAllocator(DynamicAllocator &&allocator) noexcept
...
void *DynamicAllocator::do_allocate(std::size_t size, std::size_t alignment) {
...
void DynamicAllocator::do_deallocate(void *ptr, std::size_t size,
...
bool DynamicAllocator::do_is_equal(
    const std::pmr::memory_resource &resource) const noexcept {
  const DynamicAllocator *allocator_ptr_ =
      dynamic_cast<const DynamicAllocator *>(&resource);
  if (allocator_ptr_ == nullptr) {
    return false;
  }
  return all_blocks_ == allocator_ptr_->all_blocks_;
}
bool operator==(const DynamicAllocator::BlockData &a,
                const DynamicAllocator::BlockData &b) {
